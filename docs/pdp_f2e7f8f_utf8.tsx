...
export default function ProjectDetailPanel({ project, onClose, inline, initialDetail, userId }: Props) {
  ...
  const renderHero = () => {
    const displayPrice = d?.price_range_label || (d?.price_min_cr ? `Γé╣${d.price_min_cr} Cr Onwards` : 'Price on Request')
            ...
            <div className="md:col-span-5 pb-4 md:pb-0 md:pr-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-wider mt-1.5">
                  All Inclusive {unitTypes.length > 0 && `┬╖ Starts Γé╣${Math.min(...unitTypes.map(u => u.super_area_sqft && u.price_min_cr ? Math.round((u.price_min_cr * 10000000) / u.super_area_sqft) : Infinity).filter(v => v !== Infinity))}/sqft`}
                </p>
              </div>
