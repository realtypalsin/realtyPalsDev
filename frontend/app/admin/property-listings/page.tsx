...
export default function PropertyListingsPage() {
  ...
  function handleUpdateStatus(id: string, newStatus: string) {
    ...
    <div className="max-w-6xl mx-auto py-8">
      ...
      <div className="group flex items-center gap-3 px-4 py-3 bg-white border border-zinc-200/80 rounded-xl shadow-sm mb-6 focus-within:border-zinc-300 focus-within:shadow-md transition-all">
        ...
        <input
          ...
          placeholder="Filter by project, developer, or city..."
      ...
      <div className="bg-white rounded-xl border border-zinc-200/80 shadow-sm overflow-hidden">
        <div className="flex items-center px-4 py-3 bg-zinc-50/50 border-b border-zinc-200/80 text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">
          ...
          <div className="w-[180px] hidden md:block">Location</div>
        ...
        <div className="divide-y divide-zinc-100">
          {filtered.length === 0 ? (
            <div className="py-16 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-[14px] font-medium text-zinc-900">No listings found</p>
            </div>
          ) : (
            filtered.map((item) => (
              <div key={item.id} className="group flex items-center px-4 py-4 hover:bg-zinc-50/80 transition-colors">
                ...
                <div className="flex-1 min-w-0 pr-4">
                  ...
                  <p className="text-[12px] text-zinc-500 truncate mt-0.5">{item.developer}</p>
                  <div className="flex items-center gap-1 mt-1 text-[11px] font-medium text-zinc-400">
                    <IndianRupee size={10} /> {item.price}
                  </div>
                </div>

                <div className="w-[180px] hidden md:flex flex-col gap-1 pr-4">
