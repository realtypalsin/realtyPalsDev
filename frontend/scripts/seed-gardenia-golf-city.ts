...
async function seed() {
  console.log('🌱 Seeding Gardenia Golf City project data...')

  try {
    // 1. Find or create builder
    let builder = await prisma.builder.findFirst({
      where: { slug: 'gardenia-group' }
    })
    ...
    // 2. Find or create Gardenia Golf City project
    let project = await prisma.project.findUnique({
      where: { slug: 'gardenia-golf-city-sector-75' }
    })

    const projectData = {
      name: 'Gardenia Golf City',
      slug: 'gardenia-golf-city-sector-75',
      ...
      architect: 'Gardenia Design Studio',
      interior_designer: 'Gardenia Design Studio',
      open_space_pct: 80,
      green_rating: 'Standard Green Township',
      ...
      ai_search_keywords: ['gardenia golf city', 'golf city sector 75 noida', 'ready to move sector 75 noida', 'golf flats noida']
    }
    ...
    if (!project) {
      ...
      console.log('✓ Created Gardenia Golf City project')
    } else {
      project = await prisma.project.update({
        where: { id: project.id },
        data: projectData
      })
      console.log('✓ Updated Gardenia Golf City project')
    }
    ...
    const intelligenceJson = {
      ...
      dimensionScores: {
        ...
        legalSafety: { score: 45, status: 'Registry Blocked' }
      },
      buyerPersonas: [
        {
      ...
      transparency_checks: [
        {
          ...
          ok: true,
          details: 'Approved under RERA registration ID UPRERAPRJ11463.'
        },
        {
          label: 'Noida Authority Dues Clear',
          ok: false,
          details: 'Critical default. Builder default on dues of over ₹1,717 Cr has frozen registrations.'
        }
      ],
      investment_insights: {
        appreciation_annual: '6-8%',
        appreciation_desc: 'Muted capital appreciation due to developer legal issues and secondary-market reliance.',
        rental_yield: '3.2%',
    ...
    await prisma.decisionProfile.upsert({
      ...
      update: {
        status: 'PUBLISHED',
        decision_thesis: 'Gardenia Golf City offers a serene, golf-centric lifestyle with unmatched open green spaces and massive 80-acre microclimate living in Sector 75 Noida, but severe builder financial defaults and frozen apartment registries make it a high-risk purchase that must be approached with extreme legal caution.',
        why_buy: [
    ...
    const paymentPlanData = {
      ...
      milestones: [
        ...
        { "milestone": "On Execution of Registry & Possession", "pct": 50.0, "amt": 7900000.0, "due": "Final keys & registry", "done": false }
      ]
    }
    ...
    console.log('✅ Gardenia Golf City project seeding complete!')
