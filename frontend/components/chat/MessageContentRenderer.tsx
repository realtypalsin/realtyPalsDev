      ...
      <ReactMarkdown
        ...
        components={{
          ...
          table: ({ node, ...props }: any) => (
            <div className="my-3.5 overflow-x-auto rounded-2xl border border-slate-200/90 dark:border-zinc-800 bg-white dark:bg-[#111622] shadow-xs custom-scrollbar touch-pan-y overscroll-x-contain -mx-1 sm:mx-0">
              <table className="w-full table-auto border-collapse text-left text-xs sm:text-sm text-slate-700 dark:text-zinc-300" {...props} />
            </div>
          ),
          thead: ({ node, ...props }: any) => (
            <thead className="bg-slate-100/80 dark:bg-zinc-800/80 text-[10.5px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-zinc-300 border-b border-slate-200 dark:border-zinc-700/80" {...props} />
          ),
          th: ({ node, ...props }: any) => (
            <th className="px-2.5 sm:px-4 py-2.5 sm:py-3 font-bold text-slate-900 dark:text-slate-100 whitespace-normal sm:whitespace-nowrap break-words" {...props} />
          ),
          td: ({ node, ...props }: any) => (
