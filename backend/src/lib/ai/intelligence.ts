...
export function buildDecisionIntelligence(p: IntelligenceInput): DecisionIntelligence {
  ...
  // Raw scores (0–100) — simplified to 7 explicit scores
  const builderScore   = dna?.builder_score    ?? NEUTRAL
  const legalScore     = dna?.legal_score      ?? NEUTRAL
  const possessionScore = dna?.possession_score ?? NEUTRAL
  const priceScore     = dna?.price_score      ?? NEUTRAL
  const locationScore  = dna?.location_score   ?? NEUTRAL
  const amenityScore   = dna?.amenity_score    ?? NEUTRAL
  const overallDnaScore = dna?.overall_score   ?? NEUTRAL

  // Risk score: composite of legal + possession, hard-capped by flags
  let riskScore = clamp(legalScore * 0.5 + possessionScore * 0.5)
  ...
  const reraBasis = (() => {
    ...
    if (legalScore >= 70) return 'Project has active RERA registration'
  ...
  const valueBasis = (() => {
    if (priceScore >= 70) return 'Priced below sector average — good value headroom'
    if (priceScore >= 50) return 'Market-rate pricing for this sector'
  ...
  const lifestyleBasis = (() => {
    ...
    if (amenityScore >= 65) return 'Strong lifestyle offering for the segment'
  ...
  const dimensions: DecisionDimension[] = [
    {
      ...
      basis: builderBasis,
      status: dna?.builder_score != null ? 'Verified' : 'Estimated',
    },
    {
      ...
      status: dna?.possession_score != null ? 'Verified' : 'Estimated',
    },
    {
      key: 'locationQuality',
      ...
      basis: locationBasis,
      status: dna?.location_score != null ? 'Verified' : 'Estimated',
    },
    {
      key: 'valuePositioning',
      label: 'Value',
      score: Math.round(priceScore),
      stars: toStars(priceScore),
      description: describe(priceScore),
      basis: valueBasis,
      status: dna?.price_score != null ? 'Verified' : 'Estimated',
    },
    {
      key: 'lifestyleDepth',
      label: 'Lifestyle',
      score: Math.round(amenityScore),
      stars: toStars(amenityScore),
      description: describe(amenityScore),
      basis: lifestyleBasis,
      status: dna?.amenity_score != null ? 'Verified' : 'Estimated',
    },
    {
      key: 'legalStanding',
...
export function buildWhyNot(
  ...
  const pBuilder = pDna?.builder_score ?? NEUTRAL
  const tBuilder = tDna?.builder_score ?? NEUTRAL
  ...
  // Value gap
  const pValue = pDna?.price_score ?? NEUTRAL
  const tValue = tDna?.price_score ?? NEUTRAL
  if (tValue - pValue >= 15) {
    reasons.push({
      rank: reasons.length + 1,
  ...
  // Location gap
  const pLoc = pDna?.location_score ?? NEUTRAL
  const tLoc = tDna?.location_score ?? NEUTRAL
  if (tLoc - pLoc >= 15) {
    reasons.push({
      rank: reasons.length + 1,
      label: 'Location quality',
  ...
  // Possession gap
  const pPoss = pDna?.possession_score ?? NEUTRAL
  const tPoss = tDna?.possession_score ?? NEUTRAL
  if (tPoss - pPoss >= 15) {
    reasons.push({
      rank: reasons.length + 1,
      label: 'Delivery confidence',
  ...
  // Lifestyle gap
  const pLife = pDna?.amenity_score ?? NEUTRAL
  const tLife = tDna?.amenity_score ?? NEUTRAL
  if (tLife - pLife >= 15) {
    reasons.push({
      rank: reasons.length + 1,
...
export function buildIntelligenceCompleteness(p: IntelligenceInput): IntelligenceCompleteness {
  ...
  const builderTrust: IntelligenceStatus    = dna?.builder_score != null ? 'Verified' : (missing.push('Builder track record score'), 'Unavailable')
  const deliveryConf: IntelligenceStatus    = dna?.possession_score != null ? 'Verified' : (missing.push('Possession certainty'), 'Unavailable')
  const locationQuality: IntelligenceStatus = dna?.location_score != null ? 'Verified' : (missing.push('Locality score'), 'Unavailable')
  const valuePositioning: IntelligenceStatus = dna?.price_score != null ? 'Verified' : (missing.push('Price positioning'), 'Unavailable')
  const lifestyleDepth: IntelligenceStatus  = dna?.amenity_score != null ? 'Verified' : (missing.push('Amenity depth'), 'Unavailable')
...
export function buildBuyerPersonas(p: IntelligenceInput): BuyerPersona[] {
  ...
  const familyScore = Math.round(
    amenityScore * 0.35 + locationScore * 0.30 + builderScore * 0.20 + priceScore * 0.15,
  )
  const investorScore = Math.round(
    priceScore * 0.30 + locationScore * 0.25 + builderScore * 0.20 + legalScore * 0.15 + possessionScore * 0.10,
  )
  const luxuryScore = Math.round(
    amenityScore * 0.40 + builderScore * 0.30 + locationScore * 0.20 + priceScore * 0.10,
  )
  const nriScore = Math.round(
    builderScore * 0.30 + legalScore * 0.25 + locationScore * 0.25 + possessionScore * 0.20,
  )
  const endUserScore = Math.round(
    locationScore * 0.30 + possessionScore * 0.25 + priceScore * 0.25 + builderScore * 0.20,
  )

  function familyReasons(): string[] {
    const r: string[] = []
    if (amenityScore >= 65) r.push('Strong amenity set suitable for children and family recreation')
    ...
    if (priceScore >= 55) r.push('Competitive pricing makes it accessible for families upgrading')
  ...
  function investorReasons(): string[] {
    const r: string[] = []
    if (priceScore >= 65) r.push('Below-market pricing offers potential appreciation headroom')
    else if (priceScore >= 50) r.push('Market-rate pricing — rental yields will drive returns')
  ...
  function luxuryReasons(): string[] {
    const r: string[] = []
    if (amenityScore >= 70) r.push('Premium amenity set aligned with luxury buyer expectations')
  ...
  function nriReasons(): string[] {
    const r: string[] = []
    if (legalScore >= 65) r.push('Active RERA compliance reduces long-distance ownership risk')
  ...
  function endUserReasons(): string[] {
    ...
    if (priceScore >= 55) r.push('Value positioning works in favour of a long-term end-use decision')
...
export function buildDealBreakers(p: IntelligenceInput): DealBreaker[] {
  ...
  if (p.status === 'under_construction' && (dna?.possession_score ?? NEUTRAL) < 45) {
    items.push({
