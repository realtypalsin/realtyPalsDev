...
async function main() {
  try {
    const filePath = resolve(__dirname, '../../newProj/75/realtypals_sector77_master_data.json');
    const content = readFileSync(filePath, 'utf-8');
    const data: ProjectData[] = JSON.parse(content);

    console.log(`\n📊 Seeding ${data.length} projects from Sector 77...\n`);
    ...
    for (const item of data) {
      ...
      const projectSlug = project.slug || `${project.name.toLowerCase().replace(/\s+/g, '-')}-sector-77`;
