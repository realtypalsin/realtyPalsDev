...
async function main() {
  try {
    const filePath = resolve(__dirname, '../../newProj/75/realtypals_sector76_master_data.json');
    const content = readFileSync(filePath, 'utf-8');
    const data: ProjectData[] = JSON.parse(content);

    console.log(`\n📊 Seeding ${data.length} projects from Sector 76...\n`);
    ...
    for (const item of data) {
      ...
      // 2. PROJECT - UPSERT by name + sector
      ...
      const projectSlug = project.slug || `${project.name.toLowerCase().replace(/\s+/g, '-')}-sector-76`;

      const existingProject = await prisma.project.findFirst({
        where: {
          AND: [
            { name: { mode: 'insensitive', equals: project.name } },
            { sector: { contains: '76' } }
          ]
        }
      });
