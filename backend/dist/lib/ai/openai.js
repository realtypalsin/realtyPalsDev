...
exports.StreamStallError = exports.INFERENCE_DEFAULTS = void 0;
exports.streamWithOpenAI = streamWithOpenAI;
const openai_1 = __importDefault(require("openai"));
const MAX_TOOL_CYCLES = 4;
exports.INFERENCE_DEFAULTS = {
    maxTokens: 1500,
};
// ── Tool safety ───────────────────────────────────────────────────────────────
...
function validateToolArgs(name, args) {
    ...
    for (const [field, maxLen] of Object.entries(limits)) {
        if (typeof validated[field] === 'string' && validated[field].length > maxLen) {
            console.warn('[openai] tool arg truncated', { tool: name, field, originalLength: validated[field].length });
            validated[field] = validated[field].slice(0, maxLen);
        }
    }
    return validated;
}
...
function capToolResult(result, toolName) {
    const str = typeof result === 'string' ? result : JSON.stringify(result);
    if (str.length <= TOOL_RESULT_MAX_CHARS)
        return str;
...
exports.StreamStallError = StreamStallError;
...
async function streamWithOpenAI(system, messages, send, onToolCall, config = exports.INFERENCE_DEFAULTS) {
    ...
    async function runCompletion(currentMsgs, cycle) {
        ...
        if (toolCallName && allowTools) {
            ...
            let argsObj = {};
            try {
                argsObj = JSON.parse(toolCallArgs);
            }
            catch { /* tolerate partial/empty args */ }
