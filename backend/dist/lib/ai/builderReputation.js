...
exports.getBuilderReputation = getBuilderReputation;
/**
...
function scoreFromSignals(signals) {
    ...
    for (const s of signals) {
        if (s.type === 'positive')
            score += s.category === 'delivery' ? 8 : 5;
        if (s.type === 'negative')
            score -= s.category === 'legal' ? 12 : s.category === 'delivery' ? 10 : 6;
    }
...
function extractSignals(results) {
    ...
    const awardKeywords = ['award', 'recognized', 'credai', 'certified'];
    for (const r of results.slice(0, 6)) {
        ...
        if (deliveryKeywords.some((k) => text.includes(k)))
            category = 'delivery';
        else if (legalKeywords.some((k) => text.includes(k)))
            category = 'legal';
        else if (awardKeywords.some((k) => text.includes(k)))
            category = 'awards';
        signals.push({
...
function extractReraStatus(results) {
...
function extractDeliveryTrack(results) {
    const combined = results.map((r) => r.content.toLowerCase()).join(' ');
    ...
    if (hasDelay && !hasOnTime)
        return 'Delays reported';
    if (hasOnTime && !hasDelay)
        return 'Generally on time';
    if (hasDelay && hasOnTime)
        return 'Mixed record';
    return 'Insufficient data';
}
async function getBuilderReputation(builderName) {
    const queries = SEARCH_QUERIES(builderName);
    const settled = await Promise.all(queries.map((q) => (0, tavily_1.tavilySearch)(q, 3).then(({ results }) => results).catch(() => [])));
