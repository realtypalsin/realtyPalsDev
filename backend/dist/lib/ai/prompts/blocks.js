...
exports.buildPropertyResultsFormatBlock = buildPropertyResultsFormatBlock;
exports.buildSectorAdvisoryFormatBlock = buildSectorAdvisoryFormatBlock;
exports.buildComparisonFormatBlock = buildComparisonFormatBlock;
exports.buildExpansionBlock = buildExpansionBlock;
exports.buildProjectsBlock = buildProjectsBlock;
exports.buildSectorBlock = buildSectorBlock;
exports.buildSectorsOverviewBlock = buildSectorsOverviewBlock;
...
function buildPropertyResultsFormatBlock() {
    return `
...
function buildSectorAdvisoryFormatBlock() {
    return `
...
function buildComparisonFormatBlock() {
    return `
...
function buildExpansionBlock(expansion) {
...
function serializeProjects(projects) {
    return projects.map((p, i) => {
        ...
        const connNames = p.top_connectivity.map((c) => c.distance_km ? `${c.name} (${c.distance_km}km)` : c.name).join(', ') || 'N/A';
...
function buildProjectsBlock(exactResults, sectorCtx, expansion, nearbyResults, notFoundNames) {
...
NEVER invent nearby sector names — only suggest sectors you can verify are in the Noida/Greater Noida area.`;
    ...
    let block = '';
    if (hasExact) {
        const overBudgetCount = exactResults.filter((p) => p.budgetStatus === 'slightly_over' || p.budgetStatus === 'over').length;
    ...
    if (hasNearby && nearbyResults) {
        ...
        const overBudgetNearby = nearbyResults.filter((p) => p.budgetStatus === 'slightly_over' || p.budgetStatus === 'over').length;
...
function buildSectorBlock(ctx, intent) {
...
function buildSectorsOverviewBlock(overviews, intent) {
...
function buildIntentSummary(intent) {
    ...
    if (intent.bhk?.length)
        parts.push(`BHK: ${intent.bhk.join(' or ')}`);
    if (intent.budgetMax)
        parts.push(`Budget: up to ₹${intent.budgetMax}Cr`);
    if (intent.budgetMin)
        parts.push(`Budget from: ₹${intent.budgetMin}Cr`);
    if (intent.sector)
        parts.push(`Sector: ${intent.sector}`);
    if (intent.possession)
        parts.push(`Possession: ${intent.possession}`);
    ...
    if (intent.builderName)
        parts.push(`Builder: ${intent.builderName}`);
    if (intent.lifestyleKeywords?.length)
        parts.push(`Lifestyle signals: ${intent.lifestyleKeywords.join(', ')}`);
    return parts.length ? `Detected intent: ${parts.join(' · ')}\n` : '';
}
function buildMemorySummary(memory) {
    const parts = [];
    if (memory.bhk_preference)
        parts.push(`Past BHK preference: ${memory.bhk_preference}BHK`);
    if (memory.budget_max_cr)
        parts.push(`Past budget: ₹${memory.budget_max_cr}Cr`);
    if (memory.sector_preference)
        parts.push(`Past sector interest: ${memory.sector_preference}`);
    if (memory.viewed_slugs?.length)
        parts.push(`Previously viewed: ${memory.viewed_slugs.slice(0, 3).join(', ')}`);
