...
exports.buildDecisionIntelligence = buildDecisionIntelligence;
exports.buildWhyNot = buildWhyNot;
exports.buildIntelligenceCompleteness = buildIntelligenceCompleteness;
exports.buildBuyerPersonas = buildBuyerPersonas;
exports.buildDealBreakers = buildDealBreakers;
// ─── Internal helpers ─────────────────────────────────────────────────────────
...
function tierFromScore(score, riskFlag, legalFlag) {
    if (legalFlag)
        return 'AVOID';
    if (riskFlag && score >= 63)
        return 'WATCH';
    if (score >= 78)
        return 'STRONG_BUY';
    if (score >= 63)
...
function buildDecisionIntelligence(p) {
    const dna = p.dna;
    ...
    const overallScore = clamp(builderScore * 0.20 +
        valueScore * 0.20 +
        locationScore * 0.15 +
        riskScore * 0.15 +
        lifestyleScore * 0.10 +
    ...
    const confidence = presentCount >= 5 ? 'High' : presentCount >= 3 ? 'Medium' : 'Low';
    // Builder trust dimension
    const builderBasis = (() => {
        const b = p.builder;
        if (legalFlag)
            return 'Legal dispute flag present — verify before proceeding';
        if (b?.credai_member && (b.delivered_units ?? 0) > 1000)
            return 'CREDAI member with large delivery portfolio';
        if (b?.delivered_units && b.delivered_units > 500)
            return 'Established builder with significant delivery track record';
        if (b?.awards_count && b.awards_count > 0)
            return 'Award-winning builder';
        if (builderScore >= 65)
            return 'Strong builder credentials in this market';
        return 'Builder credentials at market average';
    })();
    // RERA dimension basis
    const reraBasis = (() => {
        if (!p.rera_number)
            return 'RERA registration not verified';
        if (reraScore >= 70)
            return 'Project has active RERA registration';
        return 'RERA registered — verify completion status';
    })();
    // Possession dimension basis
    const possessionBasis = (() => {
        if (p.status === 'ready_to_move')
            return 'Ready to move in — no delivery risk';
        if (possessionScore >= 70)
            return 'Builder has strong on-time delivery record';
        if (possessionScore >= 50)
            return 'Possession timeline has moderate certainty';
        return 'Delivery certainty is limited — review builder track record';
    })();
    // Value dimension basis
    const valueBasis = (() => {
        if (valueScore >= 70)
            return 'Priced below sector average — good value headroom';
        if (valueScore >= 50)
            return 'Market-rate pricing for this sector';
    ...
    const locationBasis = (() => {
        ...
        if (locationScore >= 70)
            return `Well-connected location in ${sector}`;
        if (locationScore >= 50)
            return `Average connectivity for ${sector}`;
    ...
    const lifestyleBasis = (() => {
        ...
        if (amenityCount >= 8)
            return `${amenityCount}+ amenities including premium facilities`;
        if (amenityCount >= 4)
            return `${amenityCount} standard amenities`;
        if (lifestyleScore >= 65)
            return 'Strong lifestyle offering for the segment';
    ...
    const tradeoffs = [];
    if (legalFlag)
        tradeoffs.push('Legal dispute flag: admin verification required before proceeding');
    if (riskFlag && !legalFlag)
        tradeoffs.push('Risk flag raised: independently verify builder track record and RERA status');
    sorted
    ...
    const bottomLine = (() => {
        ...
        if (thesis && thesis.length < 160)
            return thesis;
        if (tier === 'STRONG_BUY')
            return 'Strong all-round score — this project stands out in its segment.';
        if (tier === 'BUY')
            return 'Good fundamentals — worth serious consideration.';
        if (tier === 'HOLD')
            return 'Acceptable option — verify specific concerns before committing.';
        if (tier === 'WATCH')
            return 'Below-average signals — compare with alternatives carefully.';
...
function buildWhyNot(project, topProject) {
    const reasons = [];
...
function buildIntelligenceCompleteness(p) {
    const dna = p.dna;
    const missing = [];
    const builderTrust = dna?.builder_track_record_score != null ? 'Verified' : (missing.push('Builder track record score'), 'Unavailable');
    const deliveryConf = dna?.possession_certainty_score != null ? 'Verified' : (missing.push('Possession certainty'), 'Unavailable');
    const locationQuality = dna?.locality_score != null ? 'Verified' : (missing.push('Locality score'), 'Unavailable');
    const valuePositioning = dna?.price_position_score != null ? 'Verified' : (missing.push('Price positioning'), 'Unavailable');
    const lifestyleDepth = dna?.amenity_depth_score != null ? 'Verified' : (missing.push('Amenity depth'), 'Unavailable');
    const legalStanding = p.rera_number ? 'Verified' : (missing.push('RERA registration'), 'Estimated');
    ...
    const overallCoverage = verifiedCount >= 5 ? 'Full' : verifiedCount >= 3 ? 'Partial' : 'Limited';
    return {
        builderTrust,
...
function buildBuyerPersonas(p) {
    ...
    const familyScore = Math.round(lifestyleScore * 0.35 + locationScore * 0.30 + builderScore * 0.20 + valueScore * 0.15);
    const investorScore = Math.round(valueScore * 0.30 + locationScore * 0.25 + builderScore * 0.20 + reraScore * 0.15 + possessionScore * 0.10);
    const luxuryScore = Math.round(lifestyleScore * 0.40 + builderScore * 0.30 + locationScore * 0.20 + valueScore * 0.10);
    const nriScore = Math.round(builderScore * 0.30 + reraScore * 0.25 + locationScore * 0.25 + possessionScore * 0.20);
    const endUserScore = Math.round(locationScore * 0.30 + possessionScore * 0.25 + valueScore * 0.25 + builderScore * 0.20);
    function familyReasons() {
        const r = [];
        if (lifestyleScore >= 65)
            r.push('Strong amenity set suitable for children and family recreation');
        else
            r.push('Amenity depth is average for family needs');
        if (locationScore >= 60)
            r.push('Good school and hospital connectivity');
        else
            r.push('School connectivity requires verification');
        if (valueScore >= 55)
            r.push('Competitive pricing makes it accessible for families upgrading');
        else
            r.push('Premium pricing limits affordability for mid-segment families');
        return r.slice(0, 3);
    }
    function investorReasons() {
        const r = [];
        if (valueScore >= 65)
            r.push('Below-market pricing offers potential appreciation headroom');
        else if (valueScore >= 50)
            r.push('Market-rate pricing — rental yields will drive returns');
        else
            r.push('Premium pricing compresses near-term appreciation potential');
        if (locationScore >= 65)
            r.push('Well-connected location supports consistent rental demand');
        else
            r.push('Rental demand will depend on sector employment corridor development');
        if (riskFlag)
            r.push('Risk flag present — conduct independent RERA verification before investing');
        else if (builderScore >= 65)
            r.push('Builder reputation reduces execution risk on the investment thesis');
        else
            r.push('Builder track record is average — review past project timelines');
        return r.slice(0, 3);
    }
    function luxuryReasons() {
        const r = [];
        if (lifestyleScore >= 70)
            r.push('Premium amenity set aligned with luxury buyer expectations');
        else
            r.push('Amenity set is standard — may not meet ultra-luxury benchmarks');
        if (builderScore >= 70)
            r.push('Established builder with credibility in the premium segment');
        else
            r.push('Builder positioning is mid-market rather than luxury-focused');
        if (locationScore >= 65)
            r.push('Location confers address value for the target segment');
        else
            r.push('Sector is still developing its premium identity');
        return r.slice(0, 3);
    }
    function nriReasons() {
        const r = [];
        if (reraScore >= 65)
            r.push('Active RERA compliance reduces long-distance ownership risk');
        else
            r.push('Verify RERA completion certificate status before purchase');
        if (builderScore >= 65)
            r.push('Builder has reliable domestic track record for remote buyers');
        else
            r.push('Builder reputation warrants independent reference checks from abroad');
        if (isRTM)
            r.push('Ready-to-move eliminates construction and delivery risk');
        else if (possessionScore >= 65)
            r.push('High possession certainty reduces timeline risk for overseas buyers');
        else
            r.push('Delivery timeline carries uncertainty — factor into NRI investment decision');
        return r.slice(0, 3);
    }
    function endUserReasons() {
        const r = [];
        if (locationScore >= 65)
            r.push('Good connectivity to employment hubs and daily needs');
        else
            r.push('Connectivity meets basic needs but verify commute routes');
        if (isRTM)
            r.push('Ready to move — no waiting, no rental overlap cost');
        else if (possessionScore >= 65)
            r.push('High delivery confidence reduces move-in timeline risk');
        else
            r.push('Possession timeline is uncertain — plan for rental cost overlap');
        if (valueScore >= 55)
            r.push('Value positioning works in favour of a long-term end-use decision');
        else
            r.push('Premium pricing means lower scope for future price appreciation');
        return r.slice(0, 3);
    }
    const personas = [
        {
    ...
    if (persona) {
        ...
        if (match)
            match.stars = Math.min(5, match.stars + 1);
    }
    return personas;
}
// ─── Deal Breakers / Things to Know ──────────────────────────────────────────
function buildDealBreakers(p) {
    const dna = p.dna;
    const items = [];
