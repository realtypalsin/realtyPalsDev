...
function computeCompleteness(project) {
    ...
    if (!hasDescription)
        missing.overview.push('Project description missing (min 10 chars)');
    // 6. hero_image_url
    const hasHero = present(project.hero_image_url);
    foundational.push(hasHero);
    if (!hasHero)
        missing.images.push('Hero image not set');
    // 7. builder
    const hasBuilder = project.builder != null;
    foundational.push(hasBuilder);
    if (!hasBuilder)
        missing.builder.push('Builder not assigned');
    ...
    if (!hasSuperArea) {
        ...
        if (missing_count > 0)
            missing.units.push(`${missing_count} unit type(s) missing super area (sqft)`);
    }
    ...
    if (!hasCarpetArea) {
        ...
        if (missing_count > 0)
            missing.units.push(`${missing_count} unit type(s) missing carpet area (sqft)`);
    }
    // Images (3)
    ...
    if (!hasDna)
        missing.intelligence.push(`DNA scores incomplete (${dnaScoresFilled}/6 filled, need 3+)`);
    // Competitors (1)
    const hasCompetitors = project.competitors.length >= 1;
    enrichment.push(hasCompetitors);
    if (!hasCompetitors)
        missing.competitors.push('No competitor projects added');
