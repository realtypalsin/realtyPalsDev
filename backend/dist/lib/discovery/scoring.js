 ...
 */
function computeBudgetStatus(unitTypes, intent) {
    if (!intent.budgetMax)
        return undefined;
    ...
    if (lowestPrice <= intent.budgetMax)
        return 'within';
    if (lowestPrice <= intent.budgetMax * constants_1.BUDGET_TOLERANCE_WARN)
 ...
 *   over                −10 pts
 */
function scoreProject(p, intent, budgetStatus) {
    let score = 0;
    ...
    if (intent.possession && p.possession_date) {
        ...
        if (intent.possession === 'immediate' && months <= 3)
            score += 20;
        else if (intent.possession === '1year' && months <= 12)
            score += 20;
        else if (intent.possession === '1year' && months <= 18)
            score += 14;
        else if (intent.possession === '2year' && months <= 24)
            score += 20;
        else if (intent.possession === '2year' && months <= 30)
            score += 14;
    ...
    if (intent.budgetMax && budgetStatus === 'within') {
        ...
        if (prices.length) {
            ...
            score += Math.round(Math.min(headroom, constants_1.HEADROOM_CAP) * 15);
        }
        else {
    ...
    if (intent.areaMin || intent.areaMax) {
        const fits = p.unit_types.some((u) => u.carpet_area_sqft != null &&
    ...
    if (intent.lifestyleKeywords?.length) {
        ...
        const matchCount = intent.lifestyleKeywords.filter((kw) => searchText.includes(kw.toLowerCase())).length;
    ...
    if (p.builder.credai_member)
        score += 2;
    if ((p.builder.delivered_units ?? 0) > 0)
        score += 1;
    ...
    if (p.persona_profile && intent.purpose) {
        ...
        if (intent.purpose === 'investment') {
            if (primary === 'INVESTOR')
                score += 5;
            else if (secondary.includes('INVESTOR'))
                score += 2;
        }
        else {
            const END_USE = new Set(['FAMILY', 'PROFESSIONAL', 'UPGRADER', 'RETIREE', 'NRI']);
            if (END_USE.has(primary))
                score += 5;
            else if (secondary.some((s) => END_USE.has(s)))
                score += 2;
        }
    }
    // ── Risk penalties ───────────────────────────────────────────────────
    ...
    if (intent.riskProfile) {
        ...
        const hasInsolvencyRisk = legalText.includes('nclt') || legalText.includes('insolvency') ||
...
function buildMatchReason(p, intent, budgetStatus) {
 ...
 */
function buildMatchSignals(p, intent, budgetStatus) {
    const matchReasons = [];
    const concerns = [];
    // ── Match reasons ──────────────────────────────────────────────────────────
    // BHK config
    ...
    if (budgetStatus === 'within' && intent.budgetMax) {
        const prices = p.unit_types.map(u => u.price_min_cr).filter((x) => x != null);
        if (prices.length) {
            const lowest = Math.min(...prices);
            const headroomPct = Math.round(((intent.budgetMax - lowest) / intent.budgetMax) * 100);
            if (headroomPct >= 15)
                matchReasons.push(`₹${lowest.toFixed(2)}Cr — good value vs your budget`);
            else
    ...
    // RERA registered
    if (p.rera_number)
        matchReasons.push('RERA registered');
    ...
    if (p.status === 'READY_TO_MOVE') {
        ...
        const months = (p.possession_date.getTime() - Date.now()) / (1000 * 60 * 60 * 24 * 30);
        if (months <= 6)
            matchReasons.push('possession in 6 months');
        else if (months <= 12)
            matchReasons.push('possession within a year');
    }
    ...
    if (p.builder?.credai_member)
        matchReasons.push('CREDAI member builder');
    else if ((p.builder?.delivered_units ?? 0) > 0)
        matchReasons.push('established builder');
    // Recommendation tier
    if (p.recommendation_profile?.tier === 'STRONG_BUY')
        matchReasons.push('strong buy');
    else if (p.recommendation_profile?.tier === 'BUY')
    ...
    // Risk flags
    if (p.project_risk_flag)
        concerns.push(p.project_risk_flag);
    if (p.builder?.legal_flag)
        concerns.push(p.builder.legal_flag);
    ...
    if (!hasPrices)
        concerns.push('pricing not disclosed');
