...
async function main() {
    ...
    console.log('='.repeat(70));
    console.log();
    console.log(`SCORE_THRESHOLD = ${constants_1.SCORE_THRESHOLD}`);
    console.log(`MAX_RESULTS = ${constants_1.MAX_RESULTS}`);
    console.log(`BUDGET_TOLERANCE_MAX = ${constants_1.BUDGET_TOLERANCE_MAX}`);
    console.log();
    // ── Step 1: verify all Sector 150 projects in DB ──────────────────────────
    console.log('── STEP 1: All Sector 150 projects in DB ─────────────────────');
    const allSector150 = await db_1.prisma.project.findMany({
    ...
    for (const p of allSector150) {
        ...
        const budgetStatus = (0, scoring_1.computeBudgetStatus)(p.unit_types, IDEAL_INTENT);
        const score = (0, scoring_1.scoreProject)({
        ...
        const passesThreshold = score >= constants_1.SCORE_THRESHOLD;
        console.log(`  ${p.name}`);
        console.log(`    price_min_cr: ${minPrice ?? 'NULL'}  budgetFilter (≤${(IDEAL_INTENT.budgetMax * constants_1.BUDGET_TOLERANCE_MAX).toFixed(2)}Cr): ${inBudgetFilter ? '✓' : '✗'}`);
        console.log(`    budgetStatus: ${budgetStatus}`);
        console.log(`    score: ${score} (threshold: ${constants_1.SCORE_THRESHOLD}) → ${passesThreshold ? '✓ PASS' : '✗ DROP'}`);
    ...
    const budgetPassed = allSector150.filter(p => {
        ...
        if (!prices.length)
            return false;
        return Math.min(...prices) <= IDEAL_INTENT.budgetMax * constants_1.BUDGET_TOLERANCE_MAX;
    });
    ...
    console.log('IntentState:', (0, intent_1.getIntentState)(IDEAL_INTENT));
    const idealResult = await (0, projects_1.discoverProjects)(IDEAL_INTENT);
    ...
    console.log('IntentState:', (0, intent_1.getIntentState)(BAD_INTENT_1));
    const bad1Result = await (0, projects_1.discoverProjects)(BAD_INTENT_1);
    ...
    const bad2Result = await (0, projects_1.discoverProjects)(BAD_INTENT_2);
