...
exports.getBuilderRecord = getBuilderRecord;
const db_1 = require("./db");
async function getBuilderRecord(name) {
    const builder = await db_1.prisma.builder.findFirst({
    ...
    const b = builder;
    const dataGaps = [];
    if (!builder.delivered_units)
        dataGaps.push('delivery count unverified');
    if (!b.delivery_score)
        dataGaps.push('delivery score unverified');
    if (!b.construction_quality_score)
        dataGaps.push('construction quality unverified');
    if (!b.rera_compliance_score)
        dataGaps.push('RERA compliance score unverified');
    if (b.litigation_count === null || b.litigation_count === undefined)
        dataGaps.push('litigation count unverified');
    return {
        // Identity
        name: builder.name,
        slug: builder.slug,
