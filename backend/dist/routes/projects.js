...
const score_1 = require("../lib/recommendation/score");
const router = (0, express_1.Router)();
const QuerySchema = zod_1.z.object({
    sector: zod_1.z.string().optional(),
    city: zod_1.z.string().optional(),
    bhk: zod_1.z.string().regex(/^\d+$/, 'bhk must be an integer').optional(),
    budget_max_cr: zod_1.z.string().regex(/^\d+(\.\d+)?$/, 'budget_max_cr must be a number').optional(),
...
router.get('/:slug', async (req, res) => {
    ...
    const recommendation_score = (0, score_1.computeRecommendationScore)({
...
router.get('/:slug/payment-plan', async (req, res) => {
    const project = await db_1.prisma.project.findUnique({
        where: { slug: req.params.slug },
        select: { id: true },
    });
    ...
    const plan = await db_1.prisma.paymentPlan.findUnique({
        where: { project_id: project.id },
    });
    if (!plan) {
        res.json({
...
router.get('/:slug/cost-sheet', async (req, res) => {
    const project = await db_1.prisma.project.findUnique({
        where: { slug: req.params.slug },
        select: {
            id: true,
    ...
    const sheet = await db_1.prisma.costSheet.findUnique({
        where: { project_id: project.id },
    });
    if (!sheet) {
        res.json({
    ...
    const refUnit = project.unit_types.find((u) => u.price_min_cr != null) ?? null;
    const baseCr = refUnit?.price_min_cr ?? null;
    const sqft = refUnit?.super_area_sqft ?? null;
    let totalCostBreakdown = null;
    if (baseCr != null) {
...
router.get('/:slug/investment', async (req, res) => {
    const project = await db_1.prisma.project.findUnique({
        where: { slug: req.params.slug },
        select: {
            id: true,
    ...
    const potentialAppreciation = (() => {
        if (locationScore == null || valueScore == null)
            return null;
        if (locationScore >= 70 && valueScore >= 65)
            return 'Strong';
        if (locationScore >= 55 && valueScore >= 50)
            return 'Moderate';
        return 'Weak';
    })();
    res.json({
