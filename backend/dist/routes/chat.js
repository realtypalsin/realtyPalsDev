...
const intent_1 = require("../lib/ai/intent");
const discovery_1 = require("../lib/discovery");
const confidence_1 = require("../lib/discovery/confidence");
const memory_1 = require("../lib/ai/memory");
const context_1 = require("../lib/ai/context");
const compression_1 = require("../lib/ai/compression");
const index_1 = require("../lib/ai/prompts/index");
const groq_1 = require("../lib/ai/groq");
// import { streamWithClaude } from '../lib/ai/claude'
const openai_1 = require("../lib/ai/openai");
const auth_1 = require("../lib/auth");
const request_1 = require("../lib/request");
const builders_1 = require("../lib/builders");
...
function canReuseCache(intent, prevIntent, cached) {
    ...
    if ((intent.projectNames?.length ?? 0) > 0) {
        const missing = (intent.projectNames ?? []).filter((n) => !cached.some((p) => (0, discovery_1.matchesProjectName)(n, p.name)));
        if (missing.length > 0) {
    ...
    if (intent.sector !== undefined &&
        intent.sector !== prev.sector &&
        !(0, discovery_1.isCityLevel)(intent.sector)) {
    ...
    if ((intent.bhk?.length ?? 0) > 0 && !sameSet(intent.bhk, prev.bhk ?? [])) {
        return { reuse: false, reason: 'CACHE_REJECTED', budgetOnly: false };
    }
    ...
    if (budgetChanged)
        return { reuse: true, reason: 'CACHE_REUSED', budgetOnly: true };
...
function trimMessagesToBudget(systemPrompt, msgs) {
...
const BodySchema = zod_1.z.object({
    ...
    intent: zod_1.z.record(zod_1.z.unknown()).optional(),
});
const guardrails_1 = require("../lib/ai/guardrails");
...
router.post('/', async (req, res) => {
    ...
    const prevIntent = (parsed.data.intent ?? {});
    let message = action.type === 'TEXT_MESSAGE' ? action.payload.text : '';
    ...
    const [byKey, byIp] = await Promise.all([
        (0, cache_1.checkRateLimit)(rlKey),
        (0, cache_1.checkRateLimit)(`ip:${ip}`, 40, 60),
    ]);
    ...
    const guardrailCheck = await (0, guardrails_1.inputGuardrail)(message || JSON.stringify(action.payload));
    ...
    try {
        ...
        if (action.type === 'INTENT_PATCH') {
            ...
            const { mergeIntent } = await Promise.resolve().then(() => __importStar(require('../lib/ai/intent')));
            ...
            const fieldToRemove = action.payload.field;
            const newIntent = { ...prevIntent };
            delete newIntent[fieldToRemove];
            rawIntentResult = { intent: newIntent, degraded: false };
        }
        else if (action.type === 'TEXT_MESSAGE' && message) {
            console.log('[CHAT] TEXT_MESSAGE — running LLM extraction');
            rawIntentResult = await (0, intent_1.extractIntent)(message, prevIntent);
        }
        const [memory, sessionData] = await Promise.all([
            (0, memory_1.getMemory)(userId, guestToken),
            sessionId ? db_1.prisma.chatSession.findUnique({
                where: { id: sessionId },
                select: {
                    id: true,
                    user_id: true,
        ...
        if (sessionData && !((userId && sessionData.user_id === userId) ||
        ...
        const cachedProjectsFromSession = sessionData?.last_projects
            ? sessionData.last_projects
            : null;
        ...
        intent = (!rawIntent.purpose &&
            (rawIntent.riskProfile === 'retiree' || rawIntent.riskProfile === 'first_time_buyer')) ? { ...rawIntent, purpose: 'endUse' } : rawIntent;
        ...
        intentState = (0, discovery_1.getIntentState)(intent, skipForCachedQuery);
        ...
        send('ui_state', preSearchUiState);
        ...
        let discoveryExpansion = undefined;
        let notFoundNames = undefined;
        let disambiguationText = null;
        ...
        const needsClarification = intentState === 'GATHERING' && (((intent.bhk?.length ?? 0) > 0 && !intent.sector && !intent.budgetMax && !(intent.lifestyleKeywords?.length ?? 0)) ||
            (!!intent.budgetMax && !intent.sector && !(intent.bhk?.length ?? 0) && !(intent.lifestyleKeywords?.length ?? 0)) ||
            (!!intent.sector && !(0, discovery_1.isCityLevel)(intent.sector) && !(intent.bhk?.length ?? 0) && !intent.budgetMax && !(intent.lifestyleKeywords?.length ?? 0)));
        ...
        const isAdvisoryQuery = !skipForCachedQuery && !needsClarification && intentState === 'GATHERING' && ((intent.bhk?.length ?? 0) > 0 ||
            !!intent.budgetMax ||
        ...
        const sectorsOverview = isAdvisoryQuery
            ? await (0, discovery_1.getAllSectorsOverview)(intent.lifestyleKeywords)
            : null;
        if (isAdvisoryQuery) {
            console.log('[CHAT] END getAllSectorsOverview', Date.now());
        }
        const discoverySkipReason = needsClarification ? 'needsClarification' :
        ...
        if (skipForCachedQuery) {
            ...
            const discoveryResult = await (0, discovery_1.discoverProjects)(intent);
        ...
        send('ui_state', postSearchUiState);
        ...
        const sectorCtx = sectorForContext ? await (0, discovery_1.getSectorContext)(sectorForContext) : null;
        ...
        const { messages: compressedHistory, newSummary } = await (0, compression_1.maybeCompress)(chatHistory, existingSummary);
        console.log('[CHAT] END maybeCompress', Date.now(), { compressedLen: compressedHistory.length, newSummary: !!newSummary });
        const { systemSuffix, messages: rawMessages } = (0, context_1.buildContextMessages)(message, compressedHistory, newSummary ?? existingSummary, memory);
        const systemPrompt = (0, index_1.buildAdvisorSystemPrompt)(intent, projects, memory, sectorCtx ?? undefined, sectorsOverview ?? undefined, discoveryExpansion ?? undefined, nearbyProjects.length > 0 ? nearbyProjects : undefined, notFoundNames) + systemSuffix;
        ...
        if (needsClarification) {
            const confidence = (0, confidence_1.computeConfidence)(intent);
            const clarification = (0, confidence_1.buildClarificationOptions)(intent);
        ...
        if (!needsClarification && disambiguationText === null) {
            ...
            */
            ...
            try {
                ...
                fullText = await (0, openai_1.streamWithOpenAI)(systemPrompt, messages, send, async (name, args) => {
                    if (name === 'builder_lookup') {
                        const rec = await (0, builders_1.getBuilderRecord)(args.name ?? '');
                        return rec ?? {
                            found: false,
                    ...
                    if (name === 'web_search') {
                        const ctx = await (0, web_1.webSearch)(args.query ?? '', 3);
                        return ctx
                    ...
                    if (name === 'area_info') {
                        const info = await (0, web_1.areaInfo)(args.sector ?? '', args.city ?? 'Noida');
                    ...
                    if (name === 'rera_check') {
                        ...
                        const content = await (0, web_1.readPage)(url, 2000);
                        return content
                    ...
                    if (name === 'commute') {
                        const r = await (0, web_1.commute)(args.origin ?? '', args.destination ?? '');
                    ...
                    if (name === 'calculate_emi') {
                        ...
                        const r = (0, calculators_1.calcEmi)(pCr, aRate, tYears);
                        return {
                            monthly_emi: (0, calculators_1.formatInr)(r.emi),
                            total_payment: (0, calculators_1.formatInr)(r.totalPayment),
                            total_interest: (0, calculators_1.formatInr)(r.totalInterest),
                    ...
                    if (name === 'calculate_stamp_duty') {
                        ...
                        const r = (0, calculators_1.calcStampDuty)(pCr, g);
                        return { stamp_duty: (0, calculators_1.formatInr)(r.stampDuty), registration: (0, calculators_1.formatInr)(r.registration), total: (0, calculators_1.formatInr)(r.total), rate_pct: r.rate };
                    }
                    if (name === 'calculate_gst') {
                        ...
                        const r = (0, calculators_1.calcGst)(pCr, st, cSqm);
                        return { gst: (0, calculators_1.formatInr)(r.gst), rate_pct: r.rate, category: r.category };
                    }
                ...
                console.warn('[chat] OpenAI stream failed:', err.message);
                ...
                if (err instanceof openai_1.StreamStallError && err.tokensSent) {
                ...
                if (process.env.GROQ_API_KEY) {
                    ...
                    fullText = await (0, groq_1.streamWithGroq)(fallbackSystemPrompt, messages, send);
        ...
        if (fullText) {
            ...
            (0, guardrails_1.outputGuardrail)(fullText, systemPrompt).then((gr) => {
        ...
        const responseMode = isComparison ? 'comparison' :
        ...
        if (isNewSession) {
            ...
            persistPromises.push(db_1.prisma.chatSession.create({
                ...
                if (userId)
                    (0, cache_1.invalidateSessionList)(userId).catch(() => { });
                return db_1.prisma.chatMessage.createMany({
                    data: [
                        {
                            session_id: newId,
                            role: 'user',
            ...
            persistPromises.push(db_1.prisma.chatSession.update({
                ...
                data: {
                    ...
                    ...(() => {
                        const tagged = [
                            ...projects.map((p) => ({ ...p, cacheSource: 'exact' })),
                            ...nearbyProjects.map((p) => ({ ...p, cacheSource: 'nearby' })),
                        ];
                        return tagged.length > 0 ? { last_projects: tagged } : {};
                    })(),
                },
            }));
            persistPromises.push(db_1.prisma.chatMessage.createMany({
                data: [
                    {
        ...
        if (slugsToMemorize.length > 0) {
            persistPromises.push((0, memory_1.upsertMemory)(userId, guestToken, intent, slugsToMemorize));
        }
...
router.get('/session/list', async (req, res) => {
    ...
    if (!userId && guestToken) {
        try {
            const sessions = await db_1.prisma.chatSession.findMany({
    ...
    try {
        ...
        if (result.length > 0) {
            await (0, cache_1.setCached)(cacheKey, result, SESSION_LIST_TTL);
        }
...
router.get('/session', async (req, res) => {
    ...
    if (!session) {
        session = await db_1.prisma.chatSession.create({
            data: { user_id: userId },
            include: { messages: { orderBy: { created_at: 'desc' }, take: MAX_MESSAGES } },
        });
    }
...
router.patch('/session/:id', async (req, res) => {
    ...
    await db_1.prisma.chatSession.update({ where: { id: req.params.id }, data: { title } });
    if (session.user_id) {
        await (0, cache_1.invalidateSessionList)(session.user_id);
    }
...
router.delete('/session/:id', async (req, res) => {
    ...
    await db_1.prisma.chatSession.delete({ where: { id: req.params.id } });
    if (session.user_id) {
        await (0, cache_1.invalidateSessionList)(session.user_id);
    }
    res.json({ ok: true });
...
router.delete('/intent', async (req, res) => {
    ...
    await db_1.prisma.userMemory.deleteMany({ where: { user_id: userId } });
    const newSession = await db_1.prisma.chatSession.create({ data: { user_id: userId } });
